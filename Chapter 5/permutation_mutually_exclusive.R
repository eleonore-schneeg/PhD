seurat_AD <- readRDS("~/aim2/IMC_nature/chapter_6/seurat_AD.rds")
...
table(seurat_AD$AB_niche) #all niches 
# restrciting to amyloid only 
seurat_AD$AB_niche[seurat_AD$AB_niche=="AB_niche" & seurat_AD$NP_distance_group %in% c('NP', "<10um")]="other"
table(seurat_AD$AB_niche) #all niches 

seurat_AD <- readRDS("~/aim2/IMC_nature/chapter_6/seurat_AD.rds")
meta <- readRDS("~/aim2/IMC_nature/chapter_6/refine_plaque_size/meta.rds")
seurat_AD$NP_niche=ifelse( colnames(seurat_AD) %in% rownames(meta[which(meta$NP==1),]),"NP_niche","other")
seurat_AD$AB_niche=ifelse( colnames(seurat_AD) %in% rownames(meta[which(meta$NP==0),]),"AB_niche","other")
seurat_AD$AB_niche=ifelse(seurat_AD$AB_niche=="AB_niche"|seurat_AD$NP_niche=="NP_niche","AB_niche","other")
table(seurat_AD$NP_niche) #    7599
table(seurat_AD$AB_niche) #  9816

seurat_AD=seurat_AD[,seurat_AD$NP_niche=="other"] # removing the neuritic only plaques 
dim(seurat_AD)
...
p=ggforestplot::forestplot(
  ...
  title = "Amyloid plaque niches",
  colour = TREM2
)

p+ paletteer::scale_color_paletteer_d("wesanderson::Rushmore1",direction = -1)

saveRDS(df,"df_ALLplaques.rds")
saveRDS(df,"df_amyloid.rds")
...
seurat_AD=seurat_AD[,seurat_AD$AB_niche=="other"] # removing the amyloid only plaques 
dim(seurat_AD) # should be 79795 - 9816 = 69 979
...
saveRDS(df,"df_neuriticCELLS.rds")
...
#RERUN
seurat_obj=seurat_AD[,seurat_AD$celltype2 %in%  c("oligo PLP+","oligo Olig2+","astrocyte GFAP/S100B","neuro/tau","endothelial GLUT1+") &seurat_AD$NP_niche=="other" & seurat_AD$AB_niche=='other']
dim(seurat_obj)
...
sce_sub=sce[,sce$celltype2 %in%c("neuro/astro","micro HLADR","micro C1q+") &sce$NP_niche=="NP_niche" ]
